...
void Tracker::startTracking()
{
	...
	cv::Mat frame, frame_gray, prevGray, faceROI, eqaualizerROI;
	...
	for (;;)
	{
		...
		try {
			...
			eqaualizerROI = frame_gray(cropEqualizer);

			equalizeHist(eqaualizerROI, eqaualizerROI);

			frame_gray.copyTo(eqaualizerROI);
			...
			if (needToTrackerInit)
			{
				...
				minDist = cropToTrack.width / MIN_DISTANCE_RATIO;

				goodFeaturesToTrack(frame_gray, pointsT[1], MAX_CORNERS_COUNT, QUALITY_LEVEL, minDist, mask, 3, 3, 0, 0.04);
				if (pointsT[1].size() < 1)
				{
					needToTrackerInit = true;
					pointsT[1].clear();
			...
			if (pointsT[1].size() < MIN_CORNERS_COUNT)
			{
	...
	/////////////////////////////////////////////////////////////
		...
		if (!turnOnTracking && !detectDwellFlag)
		{
...
void Tracker::calclMousePos(cv::Point2f deltaPoint, int faceH)
{
	...
	if (!turnOnTracking)
	{
		...
		{
			if (detectDwellFlag) dwellDetecting(0);
			return;
		}
		...
		mouseX = cursorPos.x + (LONG)(deltaPoint.x * horScale * horSensitivity);
		mouseY = cursorPos.y + (LONG)(deltaPoint.y * verScale * verSensitivity);
		...
		if (detectDwellFlag) dwellDetecting(mds);
...
void Tracker::quickMouseDlg()
{
...

	{
		...
		if (smileMouseLocked) mouseTimer.start();
		...
		else // if keep smiling
		{
			...
			mMouseDlg->MouseInput(isTimer2Clicks);
			mouseTimer.stop();
...
void Tracker::timerMouseDlg()
{
	...
	if (smileAIDetectedFlag || smileGeoDetectedFlag)
	{
		...
		if (smileMouseLocked) initiateStart();
...
void Tracker::dwellDetecting(float dist)
{
	if (dist < dwellDisp)
	{
		...
		if (dwellMouseLocked) dwellTimer.start();
