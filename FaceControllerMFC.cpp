...
// FaceControllerMFC.cpp : Defines the class behaviors for the application.
//
...
// CFaceControllerMFCApp construction

CFaceControllerMFCApp::CFaceControllerMFCApp()
{
...
// The one and only CFaceControllerMFCApp object

CFaceControllerMFCApp theApp;


// CFaceControllerMFCApp initialization

BOOL CFaceControllerMFCApp::InitInstance()
{
	...
	if (!mMouseDlg.GetSafeHwnd()) // Have we created the dialog?
	{
		...
		mMouseDlg.Create(IDD_MOUSE_DIALOG);

		// hide from taskbar
		mMouseDlg.ModifyStyleEx(WS_EX_APPWINDOW, WS_EX_TOOLWINDOW);

		SetWindowPos(
			mMouseDlg, // window handle
	...
	CFaceControllerMFCDlg dlg;
	dlg.tracker.mMouseDlg = &mMouseDlg;
	dlg.mOptionsDlg.mMouseDlg = &mMouseDlg;
