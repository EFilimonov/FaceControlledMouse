...
BOOL CAdvancedTab::OnInitDialog()
{
	...
	mSlider2Threads.SetRange(1, 4);
	mSliderFaceMaxNum.SetRange(24, 50);
	mSliderFaceMinNum.SetRange(6, 22);
...
void CAdvancedTab::OnNMCustomdrawSliderEwma(NMHDR* pNMHDR, LRESULT* pResult)
{
	LPNMCUSTOMDRAW pNMCD = reinterpret_cast<LPNMCUSTOMDRAW>(pNMHDR);
	UpdateData(TRUE);
	iSliderEWMA = (float)mSliderEWMA.GetPos() / 100;
	if (iSliderEWMA < 0.01) iSliderEWMA = 0.01;

	str2.Format(L"%.*f", 2, iSliderEWMA);
	str1 = "threshold: ";
...
void CAdvancedTab::OnNMCustomdrawSliderFaceMaxnum(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (iSliderMaxNum < 14) iSliderMaxNum = 14;
