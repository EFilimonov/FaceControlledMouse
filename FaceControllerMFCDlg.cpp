...
LRESULT CFaceControllerMFCDlg::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case UWM_UPDATE_OPTIONS:
		switch (wParam)
		{
			...
			case IDCANCEL:
				...
				//needAutostart = mOptionsDlg.back_needAutostart;
				...
				//if (mOptionsDlg.back_mEditKeyboard.GetLength() > 0)
	...
	/*case ID_TIMER_LOCK:
				...
				tracker.oldCursorPos = tracker.cursorPos;
...
void CFaceControllerMFCDlg::readSerialized()
{
	...
	else
	{
		...
		//MessageBox(NULL, tracker.editKeyboard, NULL);
		/*
				std::vector<std::string> fn;
		std::string path = ek;
		cv::glob(path, fn, false);
		tracker.editKeyboard = fn[0].c_str();
		//
		*/
...
/*	// Threads
	...
	mOptionsDlg.pCAdvancedTab.mSlider2Threads.SetRange(1, 4);
...
void CFaceControllerMFCDlg::OnBnClickedButtonOcr()
{
	...
	//ShellExecute(this->GetSafeHwnd(), L"open", (LPCTSTR)tracker.editKeyboard, NULL, NULL, SW_SHOWNORMAL);
	...
	{
		MessageBox(_T("Can't open onscreen keyboard. Installation of Click2Speak is recommended."));
